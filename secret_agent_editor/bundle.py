...
from .graphics import Tileset16, Tileset8, decode_prographx_16x16, decode_prographx_8x8
...
def load_game(path: Path) -> GameBundle:
    ...
    for ep in (1, 2, 3):
        ...
        gfx8 = find_file(source, f"sam{ep}02.gfx")
        levels = find_file(source, f"sam{ep}03.gfx")
        if not gfx16 or not levels:
            continue
        tiles16 = decode_prographx_16x16(gfx16.read_bytes())
        tiles8 = decode_prographx_8x8(gfx8.read_bytes()) if gfx8 else None
        level_plain = decrypt_secret_agent(levels.read_bytes(), row_key_reset=42)
        episodes[ep] = Episode(ep, tiles16, tiles8, level_plain, levels, parse_levels(level_plain))
    if not episodes:
        if tempdir:
            tempdir.cleanup()

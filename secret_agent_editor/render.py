...
from .tile_animations import animated_tile_ref, background_variant_tile_ref
...
class SecretAgentRenderer:
    ...
    def render(
        ...
        *,
        ...
        show_bg: bool = True,
        show_fg: bool = True,
        skip_codes: set[int] | frozenset[int] | None = None,
        skip_cells: set[tuple[int, int, int, str]] | frozenset[tuple[int, int, int, str]] | None = None,
        anim_tick: int = 0,
        transparent_base: bool = False,
        cell_filter: Callable[[int, int, int, str], bool] | None = None,
    ) -> Image.Image:
        ...
        fill = (0, 0, 0, 0) if transparent_base else (0, 0, 0, 255)
        img = Image.new("RGBA", (LEVEL_W * TILE, LEVEL_H * TILE), fill)
        ...
        for raw_row in range(2, ROWS_PER_LEVEL):
            ...
            layer = "fg" if is_fg else "bg"
            for x, code in enumerate(row):
                if x == 0 and code == ord("*"):
                    continue
                if skip_codes and code in skip_codes:
                    continue
                if skip_cells and (x, target_y, code, layer) in skip_cells:
                    continue
                if cell_filter is not None and not cell_filter(x, target_y, code, layer):
                    continue
                self.draw_code(img, x, target_y, code, mapping, info.bg_code, show_unknown=show_unknown, anim_tick=anim_tick)
    ...
    def draw_code(
        ...
        *,
        ...
        if code in (0x35, 0x36, 0x37):
            bank, tile_no = background_variant_tile_ref(bg_code, code) or BACKGROUND_MAP.get(bg_code, DEFAULT_BG)
        ...
        for relx, rely, bank, tile_no in refs:
            ...
            if 0 <= tx < LEVEL_W and 0 <= ty < LEVEL_H:
                bank2, tile2 = animated_tile_ref(bank, tile_no, anim_tick)
                tile = self.ep.tiles16.get(bank2, tile2)
                if tile:

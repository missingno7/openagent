...
| 107 | Cleanup-only pass: removed dead mission update wrapper methods left after the unified interpolation/simulation loop, added a bytecode-free handoff check runner, and refreshed README/project/performance/overworld/queue docs. |
| 108 | Cleanup-only pass: split render-only interpolation, camera projection, Tk/PIL frame composition, and dynamic sprite drawing into `openagent/rendering.py`; no intended gameplay or ASM behavior changes. |
| 109 | Cleanup-only pass: moved static level-image cache refresh into `rendering.py`, added `window.py` for Tk/input/navigation, added `teleport.py` for teleporter state, and promoted raw hazard constants into `semantics.py`; no intended gameplay or ASM behavior changes. |
| 110 | Added optional render-only smoothing mode: `I` cycles off/linear/smooth and `--smooth-render` enables it at launch. Pass 114 later replaced the first three-sample curve with a shared presentation follow filter. |
| 111 | Cleanup-only pass: extracted low-level movement/collision, runtime-grid queries, barrel/platform landing/carry helpers, and actor-solid probes into `openagent/movement_collision.py`; `runtime.py` remains the fixed-step orchestration owner. |
| 112 | Bugfix pass: restored the `runtime_cell_writes_for_code` import used by `runtime_visual_ids_for_code()` in the mission hard-death tile path, and added `tools/check_runtime_hard_death_import.py` to handoff checks. |
| 113 | Rechecked raw `0x4D` triggered object `0x0271/state 0x17`: the ASM branch draws/clears the blast directly and does not allocate extra projectile-impact actors, so the old three-Explosion fan-out was removed and covered by `tools/check_landmine_no_extra_explosions.py`. |
| 114 | Refactored render interpolation into `openagent/interpolation.py`; smooth mode now uses one generic presentation follow filter for player, actors and camera, reducing camera-relative platform jitter without changing fixed-tick simulation. |

| 115 | Rechecked raw `0x40` / object `0x0131` / state `0x2B`: the branch only advances animation timers, does not call player-contact helper `0x53C4`, and the lower cel is bank9 tile 1. |

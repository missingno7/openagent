...
|---|---|---|
| Player movement | EXE-style horizontal step ramp including raw `0x4E` speed bonus, corrected unshifted `6EC1/34EA/34AF` jump/fall table, exact static fall probes, fire pose timing, dynamic-platform crossing landing; pass 130 applies the vertical fall/jump phase before the same-tick horizontal destination probe, pass 131 restores the zero first jump tick plus modulo-16 fall alignment frames, and pass 133 makes moving-platform carry skip active normal jump via `DS:6EC1` | `openagent/player_motion.py`, `openagent/runtime.py`, `openagent/animation.py`, pass 9/12/80-86/90/130/131/133 notes |
